...
import org.linlinjava.ax.core.util.SystemInfoPrinter;
import org.linlinjava.ax.db.service.AxSystemConfigService;
...
class SystemInistService {
    ...
    private void inist() {
        ...
        initConfigs();
        SystemInfoPrinter.printInfo("Ax 初始化信息", getSystemInfo());
    }
    ...
    static {
        ...
        // 订单相关配置默认值
        DEFAULT_CONFIGS.put(SystemConfig.LITEMALL_MALL_NAME, "ax");
    ...
    @Autowired
    private AxSystemConfigService axSystemConfigService;

    private void initConfigs() {
        // 1. 读取数据库全部配置信息
        Map<String, String> configs = axSystemConfigService.queryAll();
        ...
        for (Map.Entry<String, String> entry : DEFAULT_CONFIGS.entrySet()) {
            ...
            axSystemConfigService.addConfig(entry.getKey(), entry.getValue());
        }
    ...
    private Map<String, String> getSystemInfo() {
        ...
        infos.put("调试级别", environment.getProperty("logging.level.org.linlinjava.ax.wx"));

        // 测试获取application-core.yml配置信息
        infos.put(SystemInfoPrinter.CREATE_PART_COPPER + 1, "模块状态");
        infos.put("邮件", environment.getProperty("ax.notify.mail.enable"));
        infos.put("短信", environment.getProperty("ax.notify.sms.enable"));
        infos.put("模版消息", environment.getProperty("ax.notify.wx.enable"));
        infos.put("快递信息", environment.getProperty("ax.express.enable"));
        infos.put("快递鸟ID", environment.getProperty("ax.express.appId"));
        infos.put("对象存储", environment.getProperty("ax.storage.active"));
        infos.put("本地对象存储路径", environment.getProperty("ax.storage.local.storagePath"));
        infos.put("本地对象访问地址", environment.getProperty("ax.storage.local.address"));
        infos.put("本地对象访问端口", environment.getProperty("ax.storage.local.port"));

        // 微信相关信息
        infos.put(SystemInfoPrinter.CREATE_PART_COPPER + 2, "微信相关");
        infos.put("微信APP KEY", environment.getProperty("ax.wx.app-id"));
        infos.put("微信APP-SECRET", environment.getProperty("ax.wx.app-secret"));
        infos.put("微信支付MCH-ID", environment.getProperty("ax.wx.mch-id"));
        infos.put("微信支付MCH-KEY", environment.getProperty("ax.wx.mch-key"));
        infos.put("微信支付通知地址", environment.getProperty("ax.wx.notify-url"));
