...
权限控制在数据库层面涉及到三个表`ax_admin`, `ax_role`和`ax_permission`：
* ax_admin表中存在roleId字段，保存角色ID数组；
* ax_role表记录角色名称和角色介绍；
* ax_permission表记录角色所用于的权限值。
...
后端权限基于shiro来实现，相关代码见ax-admin-api模块。
...
ax-admin模块的代码基于[vue-element-admin](https://github.com/PanJiaChen/vue-element-admin)
