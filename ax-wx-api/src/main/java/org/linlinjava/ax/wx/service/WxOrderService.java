...
import org.linlinjava.ax.core.util.DateTimeUtil;
...
import org.linlinjava.ax.db.util.OrderHandleOption;
...
public class WxOrderService {
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer limit, String sort, String order) {
        ...
        List<AxOrder> orderList = orderService.queryByOrderStatus(userId, orderStatus, page, limit, sort, order);

        List<Map<String, Object>> orderVoList = new ArrayList<>(orderList.size());
        for (AxOrder o : orderList) {
            ...
            AxGroupon groupon = grouponService.queryByOrderId(o.getId());
            if (groupon != null) {
            ...
            List<AxOrderGoods> orderGoodsList = orderGoodsService.queryByOid(o.getId());
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        // 订单信息
        AxOrder order = orderService.findById(orderId);
        if (null == order) {
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            AxGrouponRules rules = grouponRulesService.queryById(grouponRulesId);
            //找不到记录
            if (rules == null) {
                return ResponseUtil.badArgument();
            }
        ...
        // 收货地址
        AxAddress checkedAddress = addressService.query(userId, addressId);
        ...
        for (AxCart checkGoods : checkedGoodsList) {
        ...
        // 添加订单商品表项
        for (AxCart cartGoods : checkedGoodsList) {
            // 订单商品
            AxOrderGoods orderGoods = new AxOrderGoods();
        ...
        // 如果使用了优惠券，设置优惠券使用状态
        if (couponId != 0 && couponId != -1) {
            AxCouponUser couponUser = couponUserService.queryOne(userId, couponId);
        ...
        //如果是团购项目，添加团购信息
        if (grouponRulesId != null && grouponRulesId > 0) {
            AxGroupon groupon = new AxGroupon();
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                //参与的团购记录
                AxGroupon baseGroupon = grouponService.queryById(grouponLinkId);
    ...
    public Object cancel(Integer userId, String body) {
        ...
        // 商品货品数量增加
        List<AxOrderGoods> orderGoodsList = orderGoodsService.queryByOid(orderId);
        for (AxOrderGoods orderGoods : orderGoodsList) {
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        AxOrder order = orderService.findBySn(orderSn);
        if (order == null) {
        ...
        //  支付成功，有团购信息，更新团购信息
        AxGroupon groupon = grouponService.queryByOrderId(order.getId());
        if (groupon != null) {
            AxGrouponRules grouponRules = grouponRulesService.queryById(groupon.getRulesId());
    ...
    public Object goods(Integer userId, Integer orderId, Integer goodsId) {
        ...
        List<AxOrderGoods> orderGoodsList = orderGoodsService.findByOidAndGid(orderId, goodsId);
        ...
        AxOrderGoods orderGoods = orderGoodsList.get(0);
    ...
    public Object comment(Integer userId, String body) {
        ...
        AxOrderGoods orderGoods = orderGoodsService.findById(orderGoodsId);
        if (orderGoods == null) {
            return ResponseUtil.badArgumentValue();
        }
        ...
        // 1. 创建评价
        AxComment comment = new AxComment();
