...
import org.linlinjava.ax.admin.service.LogHelper;
...
import static org.linlinjava.ax.admin.util.AdminResponseCode.*;
...
public class AdminAdminController {
    ...
    public Object list(String username,
        ...
        List<AxAdmin> adminList = adminService.querySelective(username, page, limit, sort, order);
        return ResponseUtil.okList(adminList);
    }

    private Object validate(AxAdmin admin) {
    ...
    @PostMapping("/create")
    public Object create(@RequestBody AxAdmin admin) {
        ...
        List<AxAdmin> adminList = adminService.findAdmin(username);
        if (adminList.size() > 0) {
    ...
    public Object read(@NotNull Integer id) {
        AxAdmin admin = adminService.findById(id);
        return ResponseUtil.ok(admin);
    }
    ...
    @PostMapping("/update")
    public Object update(@RequestBody AxAdmin admin) {
        Object error = validate(admin);
        if (error != null) {
            return error;
        }
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody AxAdmin admin) {
        Integer anotherAdminId = admin.getId();
        if (anotherAdminId == null) {
            return ResponseUtil.badArgument();
        }
        ...
        AxAdmin currentAdmin = (AxAdmin) currentUser.getPrincipal();
