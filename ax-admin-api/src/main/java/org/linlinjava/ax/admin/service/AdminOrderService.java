...
public class AdminOrderService {
    ...
    public Object list(Integer userId, String orderSn, List<Short> orderStatusArray,
        ...
        List<AxOrder> orderList = orderService.querySelective(userId, orderSn, orderStatusArray, page, limit,
                sort, order);
        return ResponseUtil.okList(orderList);
    }

    public Object detail(Integer id) {
        AxOrder order = orderService.findById(id);
        List<AxOrderGoods> orderGoods = orderGoodsService.queryByOid(id);
     ...
     * 2. 管理员登录ax管理后台点击退款操作进行订单状态修改和商品库存回库
     *
