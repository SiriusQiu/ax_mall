...
import org.linlinjava.ax.admin.util.AdminResponseCode;
...
import org.linlinjava.ax.db.service.AxPermissionService;
import org.linlinjava.ax.db.service.AxRoleService;
...
import static org.linlinjava.ax.admin.util.AdminResponseCode.ROLE_NAME_EXIST;
import static org.linlinjava.ax.admin.util.AdminResponseCode.ROLE_USER_EXIST;
...
public class AdminRoleController {
    ...
    @Autowired
    private AxRoleService roleService;
    @Autowired
    private AxPermissionService permissionService;
    @Autowired
    ...
    public Object list(String name,
        ...
        List<AxRole> roleList = roleService.querySelective(name, page, limit, sort, order);
        return ResponseUtil.okList(roleList);
    }

    @GetMapping("/options")
    public Object options() {
        List<AxRole> roleList = roleService.queryAll();
    ...
    public Object read(@NotNull Integer id) {
        AxRole role = roleService.findById(id);
        return ResponseUtil.ok(role);
    }


    private Object validate(AxRole role) {
    ...
    @PostMapping("/create")
    public Object create(@RequestBody AxRole role) {
    ...
    @PostMapping("/update")
    public Object update(@RequestBody AxRole role) {
        Object error = validate(role);
        if (error != null) {
            return error;
        }
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody AxRole role) {
        ...
        List<AxAdmin> adminList = adminService.all();
        for (AxAdmin admin : adminList) {
    ...
    public Object updatePermissions(@RequestBody String body) {
        ...
        for (String permission : permissions) {
            AxPermission axPermission = new AxPermission();
            axPermission.setRoleId(roleId);
            axPermission.setPermission(permission);
            permissionService.add(axPermission);
        }
        return ResponseUtil.ok();
    }
